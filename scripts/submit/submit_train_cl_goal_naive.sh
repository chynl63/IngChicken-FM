#!/bin/bash
# Submit Sequential CL Training (LIBERO-Goal, Naive — no pretrain, no ER) on Slurm + Singularity.
#
# Usage:
...
#
# Notes:
# - Config:       configs/cl_goal_pt.yaml  (replay.enabled: false, no pretrain weights)
# - Checkpoints:  checkpoints/cl_goal_pt/
# - Results:      results/cl_goal_pt/
# - W&B:          enabled in config (entity: ingchicken, project: baseline, group: goal)
set -euo pipefail
...
mkdir -p "${BASE}/logs" "${BASE}/checkpoints/cl_goal_pt" "${BASE}/results/cl_goal_pt"

sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=fm_cl_gol_naive
#SBATCH --partition=${PARTITION}
#SBATCH --gres=gpu:${GPU_TYPE}:${GPU_N}
#SBATCH --cpus-per-task=${CPU}
#SBATCH --mem=${MEM}
#SBATCH --time=${TIME}
#SBATCH --output=${BASE}/logs/cl_gol_naive_train_%j.out
#SBATCH --error=${BASE}/logs/cl_gol_naive_train_%j.err
...
echo "  Logs:        ${BASE}/logs/cl_gol_naive_train_<JOBID>.{out,err}"
echo "  Checkpoints: ${BASE}/checkpoints/cl_goal_pt/"
echo "  Results:     ${BASE}/results/cl_goal_pt/"
